...
> **v1.4 变更摘要**：多 Phase 达临界/超临界。B4 65→70%（Cosmos 3 统一 WFM + NC AI WFM 双工业级信号），B9 65→70%（GigaBrain-0-Small + AutoQVLA + RoboECC 三条独立信号），B3 raw 85→90%（PLD 第二独立团队验证自我改进闭环），B8 65→70%（TacVLA）。C2 20→15%（达下限）。Phase 1 85→82%（FM 多范式削弱），Phase 2 68→72%（持续超临界），Phase 3 35→42%（达临界🟡），Phase 4 54→60%（持续超临界），Phase 5 40→44%（达临界🟡）。套利 #3 窗口即将关闭。B2 保守偏误预警（20/30天无下调）。language_grounding 爆发后衰退（14d 2.53x→7d 0.46x），flow_matching 唯一 stable（0.89x）。OmniVTA 视触融合 WM 新路线。Rhoda AI $4.5B。
...
**判断**：Action Head 收敛至 Flow Matching（置信度 74%，校准后 ↓自79%）。范式空间比"FM vs AR"二元叙事更丰富：FM + AR-VLA + Discrete Diffusion + FAST 多轨并存。FM 仍领先但"唯一性"叙事被削弱。
...
- 置信度：RL 后训练突破 BC 天花板 = 81%（校准后）。⚠️ 保守偏误预警：20/30天无下调证据
...
| B2 | RL 后训练突破 BC 天花板 | 81% | 03-05 | RL 不稳定性可能无法工程化解决 |
...
| B5 | Flow Matching 主导 Action Head | **74%** ↓ | 03-22 | Discrete Diffusion 4篇并发(ICLR 2026)+AR-VLA+多范式分裂；无FM adoption全量统计 |
...
| B9 | 小模型/边缘推理可行 | **70%** ↑ | **03-25** | GigaBrain-0-Small 80%/0.13s + AutoQVLA 30% VRAM + RoboECC 362ms |
...
- ⚠️ B2 保守偏误预警：20/30 天无下调证据(03-05→03-25)，03-29 必须主动搜索反方证据
...
### Phase 1: Action Head → Flow Matching 【82% 完成】 ↓
- 13/15 独立信号
- 判断：FM 仍领先但"唯一标准"叙事被多范式并存削弱
- **反相变信号（4条）**：FAST + AR-VLA(ETH) + ICLR 2026 Discrete Diffusion 4篇并发 + HybridVLA(AR+Diffusion 统一) + A2A FM 单步推理
...
- **13.5/12 独立信号**（Cosmos 3 + NC AI WFM 新增）
...
- **时间套利窗口**：
  ...
  - #3 🔴仿真规模化 > 真实数据（~1-2月，**窗口即将关闭**——Cosmos 3 + NC AI WFM + 四大工业巨头整合）
  - #4 触觉×RL 交叉（6-12月，早期）
...
- **中国阵营**：智元(Agibot)/宇树(Unitree, 2026前两月出货5500+)/灵初(LimX)/银河通用(Galaxea)/智在无界(Boundless)/XGSynBot(Z1人形发布)
...
- ⚠️ 概念区分待办："full generative WM" vs "learned dynamics model/predictive latent model"——行业混用，B4 定义需细化
...
6. **本文档截止日期**：2026-03-25，VLA 领域每周都有重大变化

---

*生成自 KW_VLA Handbook v3 | 328+ 篇源文件 → ~16K tokens 压缩索引 | 定时任务每日 09:00 增量更新*
