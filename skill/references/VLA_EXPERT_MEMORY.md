...
| §2 动作生成范式 | 103-142 | 按需 | — |
| §3 训练范式 | 144-192 | 跳过 | — |
| §4 信念网络 ★ | 194-232 | **必读** | `docs/system/BELIEF_GRAPH.md`（完整变化历史） |
| §5 收敛地图 ★ | 234-308 | **必读** | — (本文已是最完整版本) |
| §6 触觉 | 310-334 | 跳过 | `theory/tactile/` + `theory/frontier/tactile_*` |
| §7 部署 | 336-383 | 按需 | `deployment/` |
| §8 产业 | 387-451 | 按需 | `companies/` + `memory/blog/archives/vla-social-intel/` |
| §9 当前状态 ★ | 452-545 | **必读** | `reports/weekly/` 或 `reports/biweekly/` 最新 |
| §10 深度专题 | 545-633 | 按需 | `theory/frontier/` 对应论文 |
| §11 论文速查 | 635-809 | 按需 | — |
| §12 开源基础设施 | 811-826 | 按需 | — |
| §13 产品与市场 | 828-834 | 跳过 | — |
| §14 高频面试要点 | 836-871 | 按需 | — |
| §15 校准纪律 ★ | 873-882 | **必读** | — |
...
- **判断升级**：4 个独立团队（MEM/ReMem/MemoryVLA/SOMA）= 赛道形成**确认中**。长时域记忆(>5分钟)仍是核心瓶颈。再出现 1+ 个独立工作即创建新 Phase
...
6. **本文档截止日期**：2026-05-12，VLA 领域每周都有重大变化

---

*生成自 KW_VLA Handbook v3 | 336+ 篇源文件 → ~16K tokens 压缩索引 | 定时任务每日 09:00 增量更新 | v2.0.19 2026-05-12 (cross-reference verification supplement to v2.0.18, **user-prompted "再收集更多" triggered 4 material corrections**: [1] Jim Fan WAM declaration actual date 2026-04-20 NOT 05-09 [social-intel pipeline lag 19 days + sensational framing]; [2] Jim Fan methodology "pretrain WM + action FT + RL final mile" = consolidation of existing B4 latent-WM + B2 RL trajectory, NOT new paradigm; [3] v2.0.18's "C1 7th aligned signal" classification REVOKED → re-classified as B0/B4 frontier-tier consolidation signal; C1 maintains 35% with 5pp escalation gap unchanged; [4] **4 papers missed in paper-scan blackout window 05-08~12**: EA-WM 2605.06192 / OA-WAM 2605.06481 / BioProVLA-Agent 2605.07306 / DFP 2605.07727; **WAM is established research cluster** with 7-9 teams across video-diffusion + latent + Hamiltonian backbones — Jim Fan talk is industrial PR endorsement of existing direction, not concept introduction; **Phase counters updated**: Phase 1 anti-phase 14 → 15/15 critical threshold reached (DFP one-step Wasserstein pending paper-scan full-text adjudication for FM-independence); Phase 4 WM closed-loop 30/12 → 32/12 severely supercritical 2.67× (+EA-WM +OA-WAM); Phase 5 cross-embodiment 17/12 → 18/12 (+BioProVLA-Agent); **0 net confidence changes** (B4 at 75% ceiling unchanged, B0 prior implied, C1 35% maintained); **new SOP**: external declaration-grade signals → mandatory web cross-ref [>2 independent sources + date + literal quote] before belief graph propagation [v2.0.18 footer was first violation]; **next hypothesis audit P0 amended**: remove "C1 formal escalation proposal" / add "WAM as B4 sub-route 1 internal cluster split candidate" + "Phase 1 15/15 critical DFP independence adjudication"; v2.0.18 → v2.0.19 line shift +1 → §0 source map refreshed; primary-source verification chain: Sequoia Capital official + Karpathy bearblog + BigGo Finance + Sequoia Substack + multiple arxiv abstracts. **/v2.0.18 base context inherited below**)*

*生成自 KW_VLA Handbook v3 | 332+ 篇源文件 → ~16K tokens 压缩索引 | 定时任务每日 09:00 增量更新 | v2.0.18 2026-05-12 (scheduled task run, **4-day catch-up after 05-08 zero-day** — 05-09 partial social / 05-10 empty / 05-11~12 strong social with Jim Fan WAM paradigm declaration; BELIEF_GRAPH.md unchanged at v3.6.4-incremental [05-07 paper-scanner gap-fill authority]; no confidence changes per v3 calibration discipline; Phase counters unchanged [Phase 1 14/15 / Phase 2 23/15 / Phase 3 19-20/10 strict still 0 / Phase 4 30/12 / Phase 5 17/12 / Phase 5 cand. Async 4/15 / Phase 6 cand. TTC 1/15]; conservative bias counters +4d [B0=25d, B1=8d, B2=8d, B3=8d, B4=18d, B5=42d 🔴, **B6=68d 🔴 NOW OVER 67d ceiling reflection threshold by +1d → mandatory reflection event triggered**, B7=13d soft / 58d strict ⚠️, B8=56d 🔴, B9=27d]; arxiv top-lab silence at **day 41** history-max +19d (PI/Figure/NVIDIA Research strict absence continues — but Jim Fan Sequoia AI Ascent 05-09 paradigm declaration = strongest single signal during silence window); **#9 π0.7 3rd-party reproduction distance NOW IN CRITICAL <5-day URGENT window** at 5 days; #19 智元 6-30 方法论级 VLA ddl ~7 weeks (49 days); #18 顺丰 dep ~5.4 months with 星动纪元 5/8 顺丰-led $200M as first directional evidence; #22 LWD 84d; #23 VLA-TTC 86d; **🔴 HEADLINE: Jim Fan WAM paradigm declaration = C1 escalation review key evidence (7th aligned independent signal); next hypothesis audit ~05-13~14 elevated from "pending" to "C1 formal escalation proposal"**; Meta Llama-3-Robot epistemic delay now 9 days; signal decay sequence partial-terminate [05-08 zero → 05-09 weak social → 05-10 empty → 05-11~12 strong social] though paper-scan reports remain absent 5 consecutive days; **August = 2026's most critical industrial-deliverable verification month** at ~82 days; 8+ pending agenda items at hypothesis audit including B6 67d threshold adjudication and WAM concept observation candidacy v3.6.5; Source Map line ranges refreshed for v2.0.18 line shifts; **scheduled task bash sandbox unavailable today** → git push manual from Windows host required for cumulative v2.0.13→v2.0.18 catch-up commit)*
