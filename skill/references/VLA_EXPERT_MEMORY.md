...
| §2 动作生成范式 | 92-131 | 按需 | — |
| §3 训练范式 | 133-181 | 跳过 | — |
| §4 信念网络 ★ | 183-219 | **必读** | `docs/system/BELIEF_GRAPH.md`（完整变化历史） |
| §5 收敛地图 ★ | 221-281 | **必读** | — (本文已是最完整版本) |
| §6 触觉 | 283-307 | 跳过 | `theory/tactile/` + `theory/frontier/tactile_*` |
| §7 部署 | 309-356 | 按需 | `deployment/` |
| §8 产业 | 358-411 | 按需 | `companies/` + `memory/blog/archives/vla-social-intel/` |
| §9 当前状态 ★ | 413-494 | **必读** | `reports/weekly/` 或 `reports/biweekly/` 最新 |
| §10 深度专题 | 496-576 | 按需 | `theory/frontier/` 对应论文 |
| §11 论文速查 | 578-716 | 按需 | — |
| §12 开源基础设施 | 718-733 | 按需 | — |
| §13 产品与市场 | 735-741 | 跳过 | — |
| §14 高频面试要点 | 743-778 | 按需 | — |
| §15 校准纪律 ★ | 780-790 | **必读** | — |
...
| B1 | 数据飞轮是核心壁垒 | **72%** ↓ | **04-26** | **04-26 v3.5.0 假设审计执行 77→72% (-5%)**：51 天保守偏误 + 反方累积达决定性强度（**2604.21192 BEHAVIOR1K 评估方法论级**揭露 SOTA 系统性高估 + 工程门槛下降 6+ 条 [FluxVLA Engine / VLA Foundry / EmbodiedMidtrain] + 人类视频替代 4 条 [LIDEA 80% / EgoVLA / SIM1 / co-training] + 论文 vs 复现 2-3x 衰减系统性现象 + π0.7 metadata 双向影响）。**Bull 反向论点**（产业兑现 11+ 条：特斯拉量产 / 智元 3C 产线 / 自变量 20 亿 / 它石 30 亿 / 银河 25 亿国资）成立但被重新解释为"产业飞轮 ≠ 学术研究核心壁垒"——产业飞轮重要性维持，但作为研究维度的"核心壁垒"叙事被工程标准化和合成数据路径削弱。预测 #12 ✅ 关闭 |
...
- ⚠️ **系统级保守偏误（04-26）**：B1 (0d) ✅ **04-26 重置**（v3.5.0 假设审计执行 -5%）/ B3 (0d) ✅ **04-26 重置**（依赖链强制 -5%）/ B4 (2d) ✅ / B6 (**52d**) / B7 (**42d**) / B8 (**40d**) 🔴 三节点持续超阈值，v3.5.0 已审查并维持，标记"已审查-趋势确认未达上调阈值"——下次触发条件：B6 端到端 humanoid 全身控制论文 / B7 统一 token 真机长时程击败解耦正统 / B8 触觉+latent 第 9 条独立方法论信号
...
- 🆕 **新预测 (04-24~04-26)**：#15 Cortex 2.0 第三方独立复现达 SOTA（到期 2026-08-22，剩 4 个月）；#16 下季度 1-2 条 latent-WM 工业级跟进 → B4 75→80%（到期 2026-07-31，剩 3 个月，Hi-WM 学术应用变体不计入）；#17 Hi-WM "post-training-without-real-robot" 范式 6 月内 Tier-1 复现 → B2 反方升格审查（到期 2026-10-25，剩 6 个月）
- ⚠️ **依赖链 04-26 隐患**：B4 (75%) > B1 (72%) 3pp ——若 B4 后续 75→80% 触发（#16 工业级跟进），需同时复审 B1 72% 是否需要重新校准（B4 = "WM 闭环加速器"价值依赖"数据策略主导"前提）
...
6. **本文档截止日期**：2026-04-26，VLA 领域每周都有重大变�