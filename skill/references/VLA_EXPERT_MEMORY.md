...
> **v1.7 变更摘要**：**B2 执行下调 81%→77%**（校准后；raw 90%→85%）——03-31 deadline 已到，27/30 天无下调证据，5+ 条反方弹药充足，正式执行。**新论文**：SOMA（双记忆 RAG + LLM 编排，冻结 VLA OOD +56.6%，inference-time 增强方案）；BeSafe-Bench（首个四域行为安全基准，最佳代理安全任务完成率 <40%，41% 成功但不安全）；CoMo（连续潜在运动 > 离散 VQ-VAE，视频预训练新表示）；Wanderland（多传感器融合几何 grounding 仿真）。**Memory VLA 趋势增强**：SOMA = 第 4 个独立记忆增强工作，赛道形成信号从"早期"→"确认中"。**安全维度新增**：BeSafe-Bench 揭示"任务成功≠安全"系统性风险。
> **v1.6 变更摘要**（03-31）：无信念节点置信度变更。关键新论文：DFM-VLA、VLA-OPD、Scaling Sim-to-Real RL、VLA-MBPO、Realtime-VLA V2、Ruka-v2。Phase 1 收敛度 82%→80%。套利 #3 窗口确认 ≤1 月。
...
6. **本文档截止日期**：2026-04-01，VLA 领域每周都有重大变化

---

*生成自 KW_VLA Handbook v3 | 332+ 篇源文件 → ~16K tokens 压缩索引 | 定时任务每日 09:00 增量更新*
