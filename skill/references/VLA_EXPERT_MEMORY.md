...
| §2 动作生成范式 | 106-145 | 按需 | — |
| §3 训练范式 | 147-195 | 跳过 | — |
| §4 信念网络 ★ | 197-235 | **必读** | `docs/system/BELIEF_GRAPH.md`（完整变化历史） |
| §5 收敛地图 ★ | 237-311 | **必读** | — (本文已是最完整版本) |
| §6 触觉 | 313-337 | 跳过 | `theory/tactile/` + `theory/frontier/tactile_*` |
| §7 部署 | 339-386 | 按需 | `deployment/` |
| §8 产业 | 388-451 | 按需 | `companies/` + `memory/blog/archives/vla-social-intel/` |
| §9 当前状态 ★ | 453-550 | **必读** | `reports/weekly/` 或 `reports/biweekly/` 最新 |
| §10 深度专题 | 552-640 | 按需 | `theory/frontier/` 对应论文 |
| §11 论文速查 | 642-816 | 按需 | — |
| §12 开源基础设施 | 818-833 | 按需 | — |
| §13 产品与市场 | 835-841 | 跳过 | — |
| §14 高频面试要点 | 843-878 | 按需 | — |
| §15 校准纪律 ★ | 880-889 | **必读** | — |
...
| ID | 信念 | 置信度 | 上次变更 | 最强反驳 |
|----|------|--------|----------|----------|
| B0 | 数据策略 > 模型架构 | **77%** ↑ | **04-17** | π0.7 (04-16) 以 Episode Metadata + Knowledge Insulation + "架构上没啥特别，功夫在数据工程"产业级背书 → 恢复 B0=B1=B2=B3=77% 父子一致性。反方：论文 vs 社区复现 2-3x 衰减系统性（LeRobot pi0 官方 30%/Lingbot 55 vs paper 更高）。C1 直接反对但单 lab 信号不触发逆共识保护 |
...
### Phase 4: World Model → 闭环实用化 【65% 完成】 ↑ 🔴严重超临界 2.33×
- **28/12 独立信号**（05-06 paper-scan v3.6.2 +RoboAlign-R1 reward-aligned WM 第 6 sub-route，从 27/12）；前序累积：**05-04 +Being-H0.7 + MotuBrain** + **WM-UniT 04-21 XPENG industrial-academic** + **Cortex 2.0 04-22 工业级 latent-WM planner** + **RoboWM-Bench 04-21 video WM benchmark** + **Hi-WM 04-23 in-WM post-training substrate** + Curiosity-Critic 04-20 + GNWM 04-19 + WAV 04-16 + Touch Dreaming 04-14 + VLA-World for AD 04-10 + ExoActor 04-29 + LaST-R1 04-30 latent reasoning
...
6. **本文档截止日期**：2026-05-14，VLA 领域每周都有重大变化

---

*生成自 KW_VLA Handbook v3 | 338+ 篇源文件 → ~17K tokens 压缩索引 | 定时任务每日 09:00 增量更新 | v2.0.20 2026-05-14 (paper-scan + hypothesis-review v3.6.6 integration — **🔴 high-agenda signal day**: 0 net confidence changes but **6 P0 agenda items + Phase 7 candidate Long-horizon Memory VLA formally created (5/12 prototype: MEM/ReMem/MemoryVLA/SOMA/PrediMem crossed v3 explicit threshold) + 3 new predictions [#24 RoboMemArena 3rd-party reproduction ≥80% by 2026-08-14, #25 cross-task-suite ≤2 token/frame VLA by 2026-08-14, #26 ≥2 non-NVIDIA WAM papers by 2026-11-14]**; paper-scan 2 high (RoboMemArena+PrediMem 2605.10921 OpenHelix-Team 05-11 dual-system + hierarchical memory + predictive coding head + 1,000+ step benchmark / OneWM-VLA 2605.07931 05-08 1 token/frame bandwidth + 14.71M LoRA + LIBERO-Long 95.6% = arbitrage window #9 5th signal narrowing 3-6 → 2-4 months) + 1 mid (Residual Latent Action WM 2605.07079) + 1 weak (SAE Congress 2026 2605.10653) + 2 retroactive (IVLR-Trace 2605.00438 + CoRAL 2605.02600); **🆘 B6 70d broke 67d ceiling reflection threshold +3d → calibration deadlock framework-level escalation**: v3 calibration penalty creates deadlock in B6/B7 (raw 75→80 yields calibrated 72% reverse-decrease), 3-option proposal A/B/C submitted to human reflection; **3-node adversarial review (B6/C1/B4) all maintained**: B6 → sub-definition split (a/b/c) / C1 escalation conditions hardened (累积 inductive bias 不再计入) / B4 → 6-tier sub-route node split (B4a-B4f); **Phase counters**: Phase 4 WM closed-loop 32/12 → 34/12 (2.83×) / Phase 7 candidate Memory VLA 5/12 / Phase 1 15/15 maintained; **conservative bias state (2026-05-14)**: B0=27d / B1=10d / B2=soft 10d / strict 43d 🔴 / B3=10d / B4=20d / B5=44d 🔴 / **B6=70d 🆘 framework escalation** / B7=15d soft / 60d strict ⚠️ / B8=58d 🔴 / B9=29d; **arxiv top-lab silence day 42** history-max +20d (PI/Figure/NVIDIA Research/DeepMind/Tesla/Apptronik/1X/Amazon strict absence continues, only Jianlan Luo LWD 05-01 partial breakthrough); **prediction countdown**: #9 → 2026-05-17 = **3 days CRITICAL** (π0.7 3rd-party match-specialist) / #19 智元 6-30 ddl = 47 days / #22 LWD = 82 days / #23 VLA-TTC = 84 days / **🆕 #24/#25 = 92 days** / **🆕 #26 = 184 days**; segment-numbering SOP 12-13th successful (4 papers filled 05-01~05-08 segment hole); B6/B7 calibration deadlock is v3 framework-level problem — EvolveCast warning "system exceeded own rule boundary"; v2.0.19 → v2.0.20 line shift ~+5-6 (changelog +1, §9 +2 daily snapshots, §9.5 +2 PrediMem/IVLR-Trace/Phase 7 elevation, §15 date refresh). **/v2.0.19 base context inherited below**)*

*生成自 KW_VLA Handbook v3 | 336+ 篇源文件 → ~16K tokens 压缩索引 | 定时任务每日 09:00 增量更新 | v2.0.19 2026-05-12 (cross-reference verification supplement to v2.0.18, **user-prompted "再收集更多" triggered 4 material corrections**: [1] Jim Fan WAM declaration actual date 2026-04-20 NOT 05-09 [social-intel pipeline lag 19 days + sensational framing]; [2] Jim Fan methodology "pretrain WM + action FT + RL final mile" = consolidation of existing B4 latent-WM + B2 RL trajectory, NOT new paradigm; [3] v2.0.18's "C1 7th aligned signal" classification REVOKED → re-classified as B0/B4 frontier-tier consolidation signal; C1 maintains 35% with 5pp escalation gap unchanged; [4] **4 papers missed in paper-scan blackout window 05-08~12**: EA-WM 2605.06192 / OA-WAM 2605.06481 / BioProVLA-Agent 2605.07306 / DFP 2605.07727; **WAM is established research cluster** with 7-9 teams across video-diffusion + latent + Hamiltonian backbones — Jim Fan talk is industrial PR endorsement of existing direction, not concept introduction; **Phase counters updated**: Phase 1 anti-phase 14 → 15/15 critical threshold reached (DFP one-step Wasserstein pending paper-scan full-text adjudication for FM-independence); Phase 4 WM closed-loop 30/12 → 32/12 severely supercritical 2.67× (+EA-WM +OA-WAM); Phase 5 cross-embodiment 17/12 → 18/12 (+BioProVLA-Agent); **0 net confidence changes** (B4 at 75% ceiling unchanged, B0 prior implied, C1 35% maintained); **new SOP**: external declaration-grade signals → mandatory web cross-ref [>2 independent sources + date + literal quote] before belief graph propagation [v2